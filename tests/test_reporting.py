...
def test_build_score_rows_preserves_ood_subtype_when_present():
    rows = [
        {
            "image_path": "toy/ood.png",
    ...
    scores_df = build_score_rows(rows, np.array([1]), np.array([0.9]), threshold=0.5)

    assert scores_df.loc[0, "ood_subtype"] == "text_watermark"
    assert "patient_id" not in scores_df.columns
